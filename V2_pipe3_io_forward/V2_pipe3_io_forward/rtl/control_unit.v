...
localparam OP_CMP  = 4'b1010; // CMP: SUB, update flags, no register write
...
always @(*) begin
    ...
    if (op27_25 == 3'b101) begin
        ...
        reg_write   = instr[24];  // BL writes R14/LR
        mem_write   = 1'b0;
        mem_to_reg  = 1'b0;
        alu_src_imm = 1'b0;
        alu_op      = ALU_ADD;
        ...
        case (opcode)
            ...
            OP_CMP: begin
                // ARM CMP is equivalent to SUBS without write-back.
                alu_op      = ALU_SUB;
                reg_write   = 1'b0;
                flag_write  = 1'b1;
                unsupported = 1'b0;
        ...
        // Single data transfer subset: LDR/STR Rd, [Rn, #imm12]
