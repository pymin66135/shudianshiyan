...
//
// 主�?结构:
//   1. �?�数定义：定义了 ROM 地�?�宽度和�?始化文件路径（已修正为正斜�?�）。
//   2. 存储器定义：根�?�深度�?�数定义 32 �?宽度的寄存器数组。
//   3. 默认�?始化：在 initial �?�中，首先将全部存储空间默认刷为 NOP 指令。
//   4. 程�?烧录：顺�?硬编�?写入传入的 97 �?�新机器�?�?列。
//   5. 外部加载：若�?�用 USE_INIT_FILE，则调用 $readmemh 覆盖当�?程�?。
//   6. 异步读�?�：通过 assign 语�?�，根�?�当�? PC 实时输出对应的指令字。
//============================================================
