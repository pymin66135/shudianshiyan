...
// Module: tb_mcu_top
// Description:
//   针对 mcu_top (V6 + Branch) 的简单测试平台。
//   在内部直接注入机器码，验证基本运算、访存与分支指令。
//
// 结构说明:
//   1. 信号声明与时钟/复位生成：产生 100MHz 时钟和初始复位。
//   2. 全局寄存器文件 (GPRF) 模拟：在 TB 中简单实现一个 64 深度
//      的 SRAM，用于配合 LDR/STR 指令测试。
//   3. 模块例化 (mcu_top)：将 DONE_PC 设为 32'h28 (第11条指令)。
//   4. 机器码注入与控制流：通过 uut.rom 层次化路径，在 t=1 时
//      直接硬编码指令阵列，绕过外部 HEX 文件。
//
// 测试流程:
//   rom[0:7] — DP 算术/逻辑 + LDR/STR
//   rom[8]   — B 无条件跳转，跳过 rom[9]，目标 rom[10]
//   rom[9]   — MOV R8,#99 (应被跳过，R8 保持 0)
//   rom[10]  — NOP，触发 DONE_PC
//============================================================
...
module tb_mcu_top();
    ...
    mcu_top #(
        ...
        .DONE_PC(32'h00000034)      // 第 11 条指令 (10 * 4 = 40 = 0x28) 为结束 PC
    ) uut (
        .clk(clk),
        .rst(rst),
        .pc_enable(pc_enable),
    ...
    initial begin
        ...
        // 目标地址计算: 当前PC(0x20) + 8 + (2 * 4) = 0x30 (十进制 48)
        // 这意味着它将直接跳到 uut.rom[12]
        uut.rom[8] = 32'hEA000002;
        
        // ==========================================
        // 死亡陷阱区 (如果跳转失败，这些数值会污染寄存器)
        // ==========================================
        // 0x24: MOV R8, #170 (0xAA) —— 应被跳过，R8 保持 0
        uut.rom[9] = 32'hE3A080AA;
        
        // 0x28: MOV R9, #187 (0xBB) —— 应被跳过，R9 保持 0
        uut.rom[10] = 32'hE3A090BB;

        // 0x2C: MOV R10, #204 (0xCC) —— 应被跳过，R10 保持 0
        uut.rom[11] = 32'hE3A0A0CC;

        // ==========================================
        // 成功着陆区
        // ==========================================
        // 0x30: MOV R11, #88 (0x58) —— 跳转成功的证明！R11 会变成 88
        uut.rom[12] = 32'hE3A0B058;

        // 0x34: NOP (触发 DONE_PC 的新终点)
        uut.rom[13] = 32'hE1A00000; 
