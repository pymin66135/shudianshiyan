...
wire        branch_taken;
assign branch_target = id_pc + 32'd8 + {{6{id_instr[23]}}, id_instr[23:0], 2'b00};
assign branch_taken  = if_id_valid && id_branch && id_cond_pass;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        // Branch squash: when B is taken, invalidate the instruction
        // that was fetched sequentially after B (it should not execute).
